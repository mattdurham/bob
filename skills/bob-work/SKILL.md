        ...
        prompt: "Run workspace detection and write .bob/state/workspace.md.
                ...
                Otherwise run detection inline:
                  ...
                  find . -not -path './.knowledge/*' -not -path './.git/*' \
                    \( -name 'SPECS.md' -o -name 'NOTES.md' -o -name 'TESTS.md' \
                       -o -name 'BENCHMARKS.md' \) 2>/dev/null | grep -q . \
                    && SPEC_DRIVEN=true
...
Spec-driven modules: [from workspace.md spec-driven section]
```
...
Spec-driven modules: [from workspace.md, or list directories with SPECS.md/NOTES.md/etc.]
```
