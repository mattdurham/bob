...
```
TaskUpdate(id: "<task-id>", status: "in_progress", owner: "<your-agent-name>")
```

The parameter is `id`, NOT `taskId`. Use the task ID exactly as shown in TaskList (e.g. `task-003`).
...
```
TaskUpdate(id: "<task-id>", status: "done", notes: "Implemented X in file Y. Tests pass.")
```

The parameter is `id`, NOT `taskId`. Status is `done`, NOT `completed`.
...
```
TaskUpdate(id: "task-123", status: "in_progress", owner: "team-coder-1")
```
...
```
TaskUpdate(id: "task-123", status: "done", notes: "Implemented auth.go + auth_test.go. Tests pass.")
```
