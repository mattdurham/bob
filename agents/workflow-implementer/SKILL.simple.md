...
**Before writing any code**, check each target directory for a `CLAUDE.md` file:

```bash
# Check for CLAUDE.md in directories you'll be modifying
find . -mindepth 2 -name "CLAUDE.md" | head -20
```

A directory is **documented** if it contains a `CLAUDE.md` with numbered invariants.

**If a module is documented, you MUST:**
- Read `CLAUDE.md` before touching any code in that directory
...
[If any documented modules were modified:]
- CLAUDE.md reviewed: ✅/❌
- CLAUDE.md updated: ✅/❌/N/A [describe what changed, or "no invariants affected"]
[If no documented modules: "N/A — no documented modules in scope"]
