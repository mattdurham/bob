...
- Detects **spec-driven modules** (directories with CLAUDE.md) and cross-references findings against stated invariants
...
Task(subagent_type: "Explore",
     ...
     prompt: "You are inventorying a Go codebase for structural analysis.
              ...
              A directory with CLAUDE.md is a spec-driven module.

              For each spec-driven module found, read CLAUDE.md and extract:
...
Task(subagent_type: "Explore",
     ...
     prompt: "Read .bob/state/go-discovery.md.
              ...
              For each spec-driven module (has CLAUDE.md):
...
Task(subagent_type: "Explore",
     ...
     prompt: "Read .bob/state/go-analysis.md.
              ...
              - Whether it is in a spec-driven module (CLAUDE.md present) and which invariants apply
...
```
Task(subagent_type: "Explore",
     description: "Consolidate findings and produce CLAUDE.md-aware recommendations",
     run_in_background: true,
     prompt: "Read .bob/state/go-discovery.md, .bob/state/go-analysis.md, and .bob/state/go-scores.md.
              ...
              - Spec-driven modules found (CLAUDE.md): N
              ...
              **`package.FunctionName`** — Score: N/100 (CRITICAL)
              ...
              If in a spec-driven module (CLAUDE.md present):
              > **Spec context:** Module `path/` has CLAUDE.md.
              ...
              - [If spec-driven: suggest adding a complexity invariant to CLAUDE.md]
              ...
              **`package/path`** — Coupling Score: N/100 (FRAGILE)
              ...
              If in a spec-driven module (CLAUDE.md present):
              > **Spec context:** Relevant invariants from CLAUDE.md: [list]

              Recommendation:
              - [Define interface at boundary X]
              - [Extract sub-package Y to reduce Ce]
              - [If spec-driven: add coupling constraints to CLAUDE.md]
              ...
              **Recursive Functions:**
              ...
              For each spec-driven module (CLAUDE.md present), a dedicated section:

              ### `path/to/module/` (spec-driven)

              **Invariants from CLAUDE.md relevant to complexity/coupling:**
              ...
              **Invariant gaps (undocumented structural properties):**
              [Properties observed in code that should be captured as invariants in CLAUDE.md]
              Example: 'All exported functions are thin wrappers — no business logic' if observed.

              **Recommendations:**
              - [Specific actions tied to CLAUDE.md invariants]
              - Proposed new invariants for CLAUDE.md (require user approval before adding):
              ...
              For each: what to do, where, why, and whether CLAUDE.md needs updating.
...
```
The analysis found N structural properties not yet captured as invariants:
...
Use `AskUserQuestion`. **NEVER add invariants to CLAUDE.md without explicit user approval.**

For each approved invariant:
- Add to the appropriate CLAUDE.md as the next numbered invariant
